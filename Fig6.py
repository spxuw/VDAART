...
for fold in range(1):
	Train_dat =  read_csv('../filtered/external_all/Median_Imputed_Train_external_covariate.csv', header=0,sep=',')
	Test_dat =  read_csv('../filtered/external_all/Median_Imputed_Test_external_covariate.csv', sep=',')
	...
	covariate_train = Train_value[:,1506:1531]
	...
	covariate_test = Test_value[:,1506:1531]

	Dict_train = {2: mirna_train, 3: mrna_train, 5: covariate_train}
	Dict_test = {2: mirna_test, 3: mrna_test, 5: covariate_test}

	N_test = len(y_test)
	N_classifier =  15

	a = [2,3,5]
	for nchoose in range(3,4):
		...
		for j in range(len(comb)):
			...
			for i in range(X_train.shape[1]):
				row_na = np.where(np.isnan(X_train[:,i])==True)
				nrow_na = np.where(np.isnan(X_train[:,i])==False)
				X_train[row_na,i] = np.median(X_train[nrow_na,i])

			for i in range(X_test.shape[1]):
				row_na = np.where(np.isnan(X_test[:,i])==True)
				nrow_na = np.where(np.isnan(X_test[:,i])==False)
				X_test[row_na,i] = np.median(X_test[nrow_na,i])
			...
			omics_combination_results.to_csv('../results/Ensemble/Median_Python_Metrics_external_covariate.csv', index=False)	
