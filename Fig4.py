...
for fold in range(5):
	Train_dat =  read_csv('../filtered/internal_all/TOBMI_Imputed_Train_Fold_'+str(fold)+'.csv', header=0,sep=',')
	Test_dat =  read_csv('../filtered/internal_all/TOBMI_Imputed_Test_Fold_'+str(fold)+'.csv', sep=',')
	Train_label =  read_csv('../filtered/internal_all/Imputed_Train_Label_Fold_'+str(fold)+'.csv', sep=',')
	Test_label =  read_csv('../filtered/internal_all/Imputed_Test_Label_Fold_'+str(fold)+'.csv', sep=',')
	...
	for nchoose in range(1,len(a)+1):
		...
		for j in range(len(comb)):
			...
			omics_combination_results.to_csv('../results/Ensemble/TOBMI_Python_Metrics_fold_3_4.csv', index=False)	
